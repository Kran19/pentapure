  ...
  onDispatchOrderSelect(id) {
    ...
    if(o) {
      ...
      const itemRows = (o.items || []).map((i, idx) => {
        ...
        let locationHtml = '';
        const locKey = `${i.productId}_${i.grade || 'NONE'}_${i.productType || 'FINISHED'}`;
        const locs = mappings[locKey] || {};
            ...
            <div style="margin-top:8px; padding:8px; background:rgba(0,0,0,0.1); border-radius:6px; font-size:0.8rem;">
              ...
              ${locKeys.map(lk => `
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
                  <span>${lk} <span style="color:var(--secondary); font-size:0.75rem;">(Avail: ${locs[lk]} kg)</span></span>
                  <input type="number" class="loc-split-qty" data-item-id="${i.id}" data-loc="${lk}" data-max="${locs[lk]}" max="${locs[lk]}" min="0" step="0.01" placeholder="Qty" style="width:80px; padding:0.4rem; border-radius:4px; background:var(--glass-bg); color:#fff; border:1px solid var(--glass-border);">
                </div>
              `).join('')}
            </div>
          `;
        ...
        if (locationUpdates.length > 0) {
          try {
            ...
            locationUpdates.forEach(u => {
              if (mappings[u.locKey] && mappings[u.locKey][u.location]) {
                mappings[u.locKey][u.location] -= u.deduct;
                if (mappings[u.locKey][u.location] <= 0) {
                  delete mappings[u.locKey][u.location];
                }
              }
            });
            localStorage.setItem('pentapure_product_locations', JSON.stringify(mappings));
          } catch(e) { console.error('Failed to update local stock locations', e); }
        }
