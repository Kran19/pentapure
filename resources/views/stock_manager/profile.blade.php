@extends(in_array(session('auth_user')['role'] ?? '', ['ADMIN', 'SUB_ADMIN', 'STOCK_MANAGER']) || str_contains(request()->path(), 'stock_manager') || str_contains(request()->path(), 'admin') ? 'layouts.admin' : 'layouts.app')
...
<div style="max-width: 600px; margin: 0 auto;">
  <div class="card" style="text-align: center; padding: 3rem 1.5rem;">
    <div style="width:100px; height:100px; background:var(--primary); border-radius:50%; margin:0 auto 1.5rem; display:flex; align-items:center; justify-content:center; font-size:2.5rem; font-weight:bold; box-shadow: 0 10px 20px rgba(79, 70, 229, 0.3); color: white;">
      {{ strtoupper(substr($authUser['name'] ?? session('auth_user')['name'] ?? 'S', 0, 1)) }}
    </div>
    <h2 style="margin-bottom: 0.5rem; font-size: 1.8rem;">{{ $authUser['name'] ?? session('auth_user')['name'] ?? 'Stock Manager' }}</h2>
    <span class="badge" style="background:rgba(79, 70, 229, 0.2); color:var(--primary-light); margin-bottom: 2.5rem; padding: 0.4rem 1rem; font-size: 0.9rem;">{{ $authUser['role'] ?? session('auth_user')['role'] ?? 'STOCK_MANAGER' }}</span>
