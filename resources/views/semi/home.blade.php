...
<div class="flex-between mb-1" style="flex-wrap:wrap; gap:10px; align-items:center;">
  ...
  <a class="btn btn-sm btn-secondary" style="width:auto; padding:0.5rem 1rem; text-decoration:none;" href="{{ url('semi/po') }}">Purchase Requests</a>
</div>
...
<div class="card mb-2" style="background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.05); padding:1rem; margin-top:1rem;">
  <div class="card-title" style="font-size:0.85rem; color:var(--primary-light); display:flex; align-items:center; justify-content:space-between; margin-bottom:12px;">
    ...
    <div style="font-size:0.7rem; color:var(--text-muted); font-weight:normal; display:flex; gap:5px;">
      <button type="button" onclick="document.getElementById('semi-scroll-container').scrollBy({left:-200, behavior:'smooth'})" style="border:1px solid #444; background:transparent; color:#ccc; border-radius:4px; cursor:pointer; padding:2px 8px;">&larr;</button>
      <button type="button" onclick="document.getElementById('semi-scroll-container').scrollBy({left:200, behavior:'smooth'})" style="border:1px solid #444; background:transparent; color:#ccc; border-radius:4px; cursor:pointer; padding:2px 8px;">&rarr;</button>
    </div>
  </div>
  <div id="semi-scroll-container" style="display:flex; overflow-x:auto; gap:12px; padding-bottom:10px; scrollbar-width:none; -ms-overflow-style:none;">
    <style>#semi-scroll-container::-webkit-scrollbar { display: none; }</style>
...
<h3 style="margin-top:1.5rem; margin-bottom:0.8rem; color:var(--primary-light);">📦 Live Semi-Finished Stock</h3>
<div class="responsive-grid">
      ...
      <div class="flex-between">
        <div>
          <div style="font-weight:600; font-size:1.1rem;">
            <a href="{{ route('product.stock.history', ['productId' => $s['productId'] ?? $s['id'], 'stage' => 'SEMI', 'grade' => $s['grade'], 'from' => 'home']) }}" style="color: inherit; text-decoration: none; border-bottom: 1px dashed rgba(255,255,255,0.5);">
              {{ $s['name'] }}
            </a>
          </div>
  ...
  @empty
    <div class="card" style="grid-column:1/-1;"><p class="text-center text-muted">No semi-finished stock recorded yet.</p></div>
  @endforelse
</div>
@endsection
