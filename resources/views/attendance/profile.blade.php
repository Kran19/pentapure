...
<div style="max-width: 600px; margin: 0 auto; padding-top:2rem;">
  <div class="card" style="text-align: center; padding: 3rem 1.5rem;">
    <div style="width:100px; height:100px; background:var(--primary); border-radius:50%; margin:0 auto 1.5rem; display:flex; align-items:center; justify-content:center; font-size:2.5rem; font-weight:bold; box-shadow: 0 10px 20px rgba(79, 70, 229, 0.3); color: white;">
      {{ substr($authUser['name'] ?? 'User', 0, 1) }}
    </div>
    <h2 style="margin-bottom: 0.5rem; font-size: 1.8rem;">{{ $authUser['name'] ?? 'User' }}</h2>
    <span class="badge" style="background:rgba(79, 70, 229, 0.2); color:var(--primary-light); margin-bottom: 2.5rem; padding: 0.4rem 1rem; font-size: 0.9rem;">{{ $authUser['role'] ?? 'ATTENDANCE' }}</span>
